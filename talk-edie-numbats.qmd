---
title: "Engage the masses"
subtitle: "A toolkit for multi-modal expression and formative assessment in diverse classrooms"
author: Fonti Kar & Michael Lydeamore
date: today
toc: false
format:
  presentation-revealjs+letterbox:
    css: theme.css
execute:
  engine: knitr
bibliography: references.bib
---

## Hi! 👋

::: {.columns}
::: {.column width="50%"}
::: {style="text-align: center;"}
![](images/fontikar-github-photo.jpeg){fig-alt="Fonti Kar" style="width: 220px; aspect-ratio: 1; object-fit: cover; border-radius: 50%;"}
:::

* I'm [\@fontikar](https://github.com/fontikar)
* CDPAE (mostly) at Caufield
* Biodiversity data wrangler 
* Lover of open source tools

:::

::: {.column width="50%"}
::: {style="text-align: center;"}
![](images/michael-lydeamore-github-photo.jpeg){fig-alt="Michael Lydeamore" style="width: 220px; aspect-ratio: 1; object-fit: cover; border-radius: 50%;"}
:::

* I'm [\@MikeLydeamore](https://github.com/MikeLydeamore)
* Chief vibes officer
* `ggplot2battles`
:::
:::


## A classroom anecdote 

::: {.columns}
::: {.column width="55%"}

You walk into your class and think to yourself: 

- "Goodness, am I getting older?" 😯
- "How am I going to compete their whatever is on their phone?" 😬
- "What does yeet/sigma/aura mean?" 😵‍💫
- "Yikes, how can I engage and build a connection with my students?" 😩
:::

::: {.column width="45%"}
::: {.fragment .custom-callout-box}
![](images/dead-poets-society.jpg){fig-alt="Dead Poets Society classroom scene" width="100%"}
What solutions do we have as educators??
:::
:::
:::


## A development anecdote

In `VSCode-R`, you can run custom commands using your highlighted text, very helpful for say `tar_load()` or `head()`.

In Positron, this is not an option, although the developers were receptive when I suggested it. So, I decided I would add it myself.

::: {.columns}
::: {.column width='50%'}
::: {.fragment}
1. Clone the repo
2. Install the dependencies
3. Run the dev task
4. ????
5. App crashes on launch
:::
:::
::: {.column width='50%'}
::: {.fragment .custom-callout-box}
This is exactly the fear students feel in our classes.
:::

:::
:::

## Our inspo ✨

At [WOMBAT 2024,](https://wombat2024.numbat.space/Developing%20tools%20for%20%E2%80%9Creal%20time%E2%80%9D%20formative%20assessment%20of%20writing%20within%20large%20introductory%20statistics%20and%20data%20science%20courses.pdf) [Anna Ferguson](https://www.stat.auckland.ac.nz/~fergusson/) showed off the "Quick Write Tool"

![](images/2026-08-03-11-44-13.png){fig-align="center"}

## Our inspo ✨

And it seems to work

![](images/2026-08-03-11-54-58.png){fig-align="center"}


## Data is stored in a GSheet

![](images/2026-08-03-11-45-11.png){fig-align="center"}

## For now (but still)

![](images/2026-08-03-11-45-36.png){fig-align="center"}

## Write-Now

Anna has released this tool openly (at ICOTS a few weeks ago)

::: {style="text-align: center"}
<https://write-now.online/>
:::

You can self host this using the linked repo and a Google Sheet.

## Ed-ie

::: {.columns}
::: {.column width='50%'}

<https://www.ed-ie.com>

:::

::: {.column width='50%'}
::: {.fragment style="text-align: center;"}
![](images/wall-e.jpeg){fig-alt="WALL-E" style="width: 260px; aspect-ratio: 1; object-fit: cover; border-radius: 50%;"}
:::
:::
:::

## Ed-ie features as educators 🍎

::: {.incremental}
* Submit a prompt: 💬
  * A pre-flight check
  * A formative assessment^[@sadler1989formative]
  * A invite for questions/feedback
* Pace the classroom ⏱️
* Review responses before sharing widely 👷
* Generate discussion and connection by sharing responses^[@garrison2000critical; @vygotsky1978mind]
:::

## Edie-features as learners 🐛


::: {.columns}
::: {.column width='50%'}
::: {.incremental}
* Engage _actively_, but ✨anonymously✨^[@chi2014icap; @freeman2006anonymity]
* Upvote each others questions
* Multi-modal reply^[@cast2024udl; @meyer2014udl]:
  * Text (emojis welcome!)
  * Draw (Tactile, visual learners)
  * GIPHY^[@habsari2021millennials]

:::
:::


::: {.column .fragment width="45%"}
![](images/drake.avif){fig-alt="Drake dancing in a music video" width="100%"}
:::
:::

## Ed-ie structure

::: {.incremental}
* Ed-ie consists of a set of _spaces_
* Each space consists of a set of _sessions_
* Each session has a unique set of data associated with it
:::

::: {.fragment}
The host page lets you log into a space, and manage your sessions.
:::

::: {.fragment}
* Participants join a space + session using the join page, or the QR code on the host page
:::

## Behind the scenes

::: {.columns}
::: {.column width='50%'}
Each comment is inserted into a SQL database

* `uuid`
* `session_code`
* text, gif_data, drawing_data
* status, starred, flagged
* name
:::
::: {.column width='50%'}
::: {.fragment}
Separate tables for:

* Votes
* Group questions
* Question banks
* Prompt history
* Spaces & sessions

:::

:::
:::

## Why not a Google Sheet?

* Tied to an account
* Not quite as good of data-type protection for long term storage
* Concurrency restrictions can make large classes perform poorly
* Lookups fall apart after about 10,000 rows

::: {.fragment}
The database is more _scalable_ but a bit less "free"
:::


## Behind the scenes

Deployment managed by Vercel, fully automated on every push to repo

Connections to database and admin passwords handled by environment variables

Generous free tier (so far)

## Seeking collaborations 🔍

* Do you have a large format classroom and would like to roll out Ed-ie? 
* Do you want to contribute to the experimental design in measuring student engagement pre/post Ed-ie? 
* Do you have a feature you want to desperately add? 
* Do you have a knack for writing ethics applications?
* Do you have art skills to give Ed-ie a face? 

::: {.fragment .custom-callout-box}
We welcome and acknowledge contributions of all shapes and sizes 😊
:::

## Cited sources
